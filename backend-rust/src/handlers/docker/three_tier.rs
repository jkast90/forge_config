...
pub(super) async fn compute_three_tier_preview(
        ...
        for r in 1..=rows_per_hall {
            ...
            for k in 1..=racks_per_row {
                ...
                if k == mid + 1 {
                    ...
                    spine_rack_names.push((spine_name, row_num.clone(), rack_index));
                    rack_index += 1;
                }
                ...
                leaf_rack_names.push((leaf_name, row_num.clone(), rack_index));
                rack_index += 1;
            }
        }
    }
    ...
    let leaf_rack_capacity = leaf_rack_names.len() * devices_per_rack;
    if access_count > leaf_rack_capacity && !leaf_rack_names.is_empty() {
        return Err(ApiError::bad_request(format!(
            "Not enough racks for {} access switches: {} leaf racks × {} devices/rack = {} capacity. \
             Increase racks per row (currently {}), rows per hall (currently {}), \
             halls (currently {}), or devices per rack (currently {})",
            access_count, leaf_rack_names.len(), devices_per_rack, leaf_rack_capacity,
            racks_per_row, rows_per_hall, hall_count, devices_per_rack
        )));
    }
    ...
    for i in 1..=dist_count {
        let (rack_name, row_num, r_idx, pos) = if !spine_rack_names.is_empty() {
            ...
            let (ref rn, ref rw, ri) = spine_rack_names[idx];
    ...
    for i in 1..=access_count {
        let (rack_name, row_num, r_idx, pos) = if !leaf_rack_names.is_empty() {
            ...
            if idx < leaf_rack_names.len() {
                let (ref rn, ref rw, ri) = leaf_rack_names[idx];
                (Some(rn.clone()), rw.clone(), Some(ri), Some(compute_rack_position(pos_in_rack, &req.tier3_placement, 42)))
            } else {
                (None, String::new(), None, None)
            }
        } else {
    ...
    let gpu_clusters = if req.gpu_cluster_count > 0 {
        ...
        for ci in 0..req.gpu_cluster_count {
            ...
            for ni in 0..req.gpu_nodes_per_cluster {
                ...
                let gpu_rack_name = access_rack_idx.and_then(|ri| racks.get(ri).map(|r| r.name.clone()));
...
pub async fn build_three_tier(
    ...
    // Resolve vendor IDs for patch-panel and AMD
        ...
        for h in 1..=hall_count {
            ...
            for r in 1..=rows_per_hall {
                ...
                let pp_req = crate::models::CreateDeviceRequest {
                    ...
                    serial_number: Some(format!("SN-HIER-{}", final_hostname)),
