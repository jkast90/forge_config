...
impl TopologyRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: &str) -> Result<Option<Topology>> {
    ...
    pub async fn update(pool: &Pool<Sqlite>, id: &str, req: &CreateTopologyRequest) -> Result<Topology> {
        ...
        if result.rows_affected() == 0 {
            return Err(super::NotFoundError::new("Topology", id).into());
        }
    ...
    pub async fn delete(pool: &Pool<Sqlite>, id: &str) -> Result<()> {
        ...
        sqlx::query("UPDATE devices SET topology_id = '', topology_role = '' WHERE topology_id = ?")
            .bind(id)
            .execute(pool)
            .await?;
