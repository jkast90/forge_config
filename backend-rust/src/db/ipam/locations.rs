...
impl IpamRegionRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: i64) -> Result<Option<IpamRegion>> {
        let row = sqlx::query(
            r#"SELECT r.*, COALESCE((SELECT COUNT(*) FROM ipam_locations WHERE region_id = r.id), 0) as campus_count
    ...
    pub async fn create(pool: &Pool<Sqlite>, req: &CreateIpamRegionRequest) -> Result<IpamRegion> {
        let now = Utc::now();
        let result = sqlx::query("INSERT INTO ipam_regions (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)")
            .bind(&req.name).bind(req.description.as_deref().unwrap_or(""))
            .bind(now).bind(now)
            .execute(pool).await?;
        let new_id = result.last_insert_rowid();
        Self::get(pool, new_id).await?.context("Region not found after creation")
    }

    pub async fn update(pool: &Pool<Sqlite>, id: i64, req: &CreateIpamRegionRequest) -> Result<IpamRegion> {
        let now = Utc::now();
        ...
        if result.rows_affected() == 0 {
            return Err(crate::db::NotFoundError::new("Region", &id.to_string()).into());
        }
...
impl IpamCampusRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: i64) -> Result<Option<IpamCampus>> {
        let row = sqlx::query(
            r#"SELECT l.*, r.name as region_name,
                      COALESCE((SELECT COUNT(*) FROM ipam_datacenters WHERE location_id = l.id), 0) as datacenter_count
               FROM ipam_locations l
    ...
    pub async fn create(pool: &Pool<Sqlite>, req: &CreateIpamCampusRequest) -> Result<IpamCampus> {
        let now = Utc::now();
        let result = sqlx::query("INSERT INTO ipam_locations (name, description, region_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
            .bind(&req.name).bind(req.description.as_deref().unwrap_or(""))
            .bind(req.region_id).bind(now).bind(now)
            .execute(pool).await?;
        let new_id = result.last_insert_rowid();
        Self::get(pool, new_id).await?.context("Campus not found after creation")
    }

    pub async fn update(pool: &Pool<Sqlite>, id: i64, req: &CreateIpamCampusRequest) -> Result<IpamCampus> {
        let now = Utc::now();
        let result = sqlx::query("UPDATE ipam_locations SET name = ?, description = ?, region_id = ?, updated_at = ? WHERE id = ?")
            ...
            .execute(pool).await?;
        if result.rows_affected() == 0 {
            return Err(crate::db::NotFoundError::new("Campus", &id.to_string()).into());
        }
...
impl IpamDatacenterRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: i64) -> Result<Option<IpamDatacenter>> {
        let row = sqlx::query(
            r#"SELECT dc.*, dc.location_id as campus_id, l.name as campus_name, r.name as region_name,
                      COALESCE((SELECT COUNT(*) FROM ipam_halls WHERE datacenter_id = dc.id), 0) as hall_count,
                      COALESCE((SELECT COUNT(*) FROM ipam_prefixes WHERE datacenter_id = dc.id), 0) as prefix_count
    ...
    pub async fn create(pool: &Pool<Sqlite>, req: &CreateIpamDatacenterRequest) -> Result<IpamDatacenter> {
        let now = Utc::now();
        let result = sqlx::query("INSERT INTO ipam_datacenters (name, description, location_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
            .bind(&req.name).bind(req.description.as_deref().unwrap_or(""))
            .bind(req.campus_id).bind(now).bind(now)
            .execute(pool).await?;
        let new_id = result.last_insert_rowid();
        Self::get(pool, new_id).await?.context("Datacenter not found after creation")
    }

    pub async fn update(pool: &Pool<Sqlite>, id: i64, req: &CreateIpamDatacenterRequest) -> Result<IpamDatacenter> {
        let now = Utc::now();
        let result = sqlx::query("UPDATE ipam_datacenters SET name = ?, description = ?, location_id = ?, updated_at = ? WHERE id = ?")
            ...
            .execute(pool).await?;
        if result.rows_affected() == 0 {
            return Err(crate::db::NotFoundError::new("Datacenter", &id.to_string()).into());
        }
...
impl IpamHallRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: i64) -> Result<Option<IpamHall>> {
        let row = sqlx::query(
            r#"SELECT h.*, dc.name as datacenter_name,
                      COALESCE((SELECT COUNT(*) FROM ipam_rows WHERE hall_id = h.id), 0) as row_count
               FROM ipam_halls h
    ...
    pub async fn create(pool: &Pool<Sqlite>, req: &CreateIpamHallRequest) -> Result<IpamHall> {
        let now = Utc::now();
        let result = sqlx::query("INSERT INTO ipam_halls (name, description, datacenter_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
            .bind(&req.name).bind(req.description.as_deref().unwrap_or(""))
            .bind(req.datacenter_id).bind(now).bind(now)
            .execute(pool).await?;
        let new_id = result.last_insert_rowid();
        Self::get(pool, new_id).await?.context("Hall not found after creation")
    }

    pub async fn update(pool: &Pool<Sqlite>, id: i64, req: &CreateIpamHallRequest) -> Result<IpamHall> {
        let now = Utc::now();
        let result = sqlx::query("UPDATE ipam_halls SET name = ?, description = ?, datacenter_id = ?, updated_at = ? WHERE id = ?")
            ...
            .execute(pool).await?;
        if result.rows_affected() == 0 {
            return Err(crate::db::NotFoundError::new("Hall", &id.to_string()).into());
        }
...
impl IpamRowRepo {
    ...
    pub async fn get(pool: &Pool<Sqlite>, id: i64) -> Result<Option<IpamRow>> {
        let row = sqlx::query(
            r#"SELECT r.*, h.name as hall_name,
                      COALESCE((SELECT COUNT(*) FROM ipam_racks WHERE row_id = r.id), 0) as rack_count
               FROM ipam_rows r
    ...
    pub async fn create(pool: &Pool<Sqlite>, req: &CreateIpamRowRequest) -> Result<IpamRow> {
        let now = Utc::now();
        let result = sqlx::query("INSERT INTO ipam_rows (name, description, hall_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
            .bind(&req.name).bind(req.description.as_deref().unwrap_or(""))
            .bind(req.hall_id).bind(now).bind(now)
            .execute(pool).await?;
        let new_id = result.last_insert_rowid();
        Self::get(pool, new_id).await?.context("Row not found after creation")
    }

    pub async fn update(pool: &Pool<Sqlite>, id: i64, req: &CreateIpamRowRequest) -> Result<IpamRow> {
        let now = Utc::now();
        let result = sqlx::query("UPDATE ipam_rows SET name = ?, description = ?, hall_id = ?, updated_at = ? WHERE id = ?")
            ...
            .execute(pool).await?;
        if result.rows_affected() == 0 {
            return Err(crate::db::NotFoundError::new("Row", &id.to_string()).into());
        }
