...
import { createChangeHandler } from '../utils/forms';
...
export interface UseModalFormOptions<TItem, TFormData extends object> {
  ...
  /** Modal route name for URL hash integration (optional) */
  modalName?: string;
}
...
export interface UseModalFormReturn<TItem, TFormData extends object> {
  ...
  /** Change handler for React form elements (input/select/textarea) */
...
export function useModalForm<TItem, TFormData extends object>({
  ...
  const openAdd = useCallback(() => {
    ...
    setIsOpen(true);
    if (modalName) modalRoute.openModal(modalName);
  }, [emptyFormData, modalName, modalRoute]);

  const openEdit = useCallback((item: TItem) => {
    setEditingItem(item);
    setFormData(itemToFormData(item));
    setIsOpen(true);
    if (modalName) modalRoute.openModal(modalName, { id: getItemId(item) });
  }, [itemToFormData, modalName, modalRoute, getItemId]);

  const close = useCallback(() => {
    setIsOpen(false);
    setEditingItem(null);
    setFormData(emptyFormData);
    if (modalName) modalRoute.closeModal();
  }, [emptyFormData, modalName, modalRoute]);
  ...
  const handleChange = useMemo(() => createChangeHandler<TFormData>(
